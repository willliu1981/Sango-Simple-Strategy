...
BGM 作者的 Suno 頁面：[willliu1981](https://suno.com/@willliu1981)。此連結為作者個人頁，不是六首曲目的專屬播放清單；音樂使用範圍以適用的 Suno 官方條款及授權為準，本專案文件提供來源與使用範圍說明。

作者允許重複利用本公開 V1 專案中其有權授權的原始碼與內容，包含修改、再散布及商業用途，條件詳見 [使用授權](LICENSE.md)。此一般許可不涵蓋下述六首 Suno BGM，亦不涵蓋私人獨立開發的 V2。
...
本次六首 BGM 由作者確認為 Suno Pro 方案正式下載；原曲由自己的帳號在 Pro 期間生成，涉及 Remix 的版本亦由自己於 Pro 期間製作。曲目與中英名稱見 [音樂清單](assets/audio/README.md)。

**BGM 說明更正（2026-09-17）：**音樂保留作為遊戲 BGM；先前概括允許他人商用與再散布的說明已調整。本專案暫不另行提供這六首音樂的第三方商用、獨立再散布或再授權許可，相關範圍仍待確認。商用衍生版本請替換音樂或另行取得適用授權。這不代表所有自製 Remix 都不能商用；完整更正與既有使用者說明見 [使用授權](LICENSE.md)。
