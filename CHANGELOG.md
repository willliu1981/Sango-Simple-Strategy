...
## 未發布 — 2026-09-17 曲名與文件更新

- 將六首 BGM 的用途說明限定為測試本遊戲中的播放、音量平衡與場景搭配效果，原始碼及其他內容的授權不變，並保留適用官方條款的說明。
- 春季曲由「遠方的燈火／Lights in the Distance」更名為「重建／Building Again」，同步遊戲顯示、曲目清單與發布文件，保留原始音檔及資產路徑。
- 補充六首 Suno BGM 的作者來源說明：原曲由本人於 Pro 期間生成，涉及 Remix 的版本亦由本人於 Pro 期間製作。
- 調整先前概括提供的音樂商用與再散布說明：保留遊戲 BGM，暫不另行提供第三方商用、獨立再散布或再授權許可，相關範圍待確認，並以適用的 Suno 官方條款及授權為準。
- 本次更新曲名與文件，不變更音檔、播放流程或原始碼授權；未宣告既有使用者可能取得的權利溯及失效。

## 0.6.3

- 替換主選單、破關與四季共六首 BGM，音樂鑑賞同步顯示中英文名稱與新曲長。
- 記錄作者提供的 Suno Pro 正式下載來源；當時新增的下載與商用許可說明已於 2026-09-17 更正，請見上方紀錄與目前的 `LICENSE.md`。
- 公開 V1 專案允許再利用；衍生 App 發布時必須更換 package、namespace 與 applicationId，第三方素材另依原授權。
- 此次版本與文件更新不包含私人 V2；不變更存檔格式。
